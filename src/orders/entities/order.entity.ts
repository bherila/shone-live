...
export class Order {
    ...
    @Column({
        comment: "datetime that order was created on stripe, in stripe this is just `created` but for us we rename so we save our created time eg 2020-12-01T00:00:00",
    })
    stripe_created: Date;

    @Column({
        comment: "datetime that order was updated on stripe, in stripe this is just `updated` but for us we rename so we save our updated time eg 2020-12-01T00:00:00",
    })
    stripe_updated: Date;
    ...
    @Column({
        comment: "the amount in cents that was returned if it wasnt the total order amount",
        nullable: true,
    })
