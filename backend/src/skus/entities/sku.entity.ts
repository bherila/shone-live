...
export class Sku {
  ...
  @ManyToOne((type) => Product, (product) => product.skus, {
  ...
  @OneToMany((type) => OrderSku, (orderSku) => orderSku.sku, {
    cascade: true,
  })
  orderSkus: OrderSku[];

  // these are SKUs a seller added
  @ManyToOne((type) => User, (user) => user.skus, {
    cascade: ["insert", "update"],
  })
  user: User;
  ...
  showId: number;
  @ManyToOne((type) => Show, (show) => show.skus, {
    cascade: ["insert", "update"],
  })
  show: Show;
}
